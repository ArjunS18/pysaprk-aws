...
def perform_data_clean(df_city, df_prescriber_fact):
    ...
    try:
        ...
        df_city_sel = df_city.select(upper(df_city.city).alias("city"),
                                 df_city.state_id,
    ...
    #Clean df_prescriber_fact DataFrame:
    #1 Select only required Columns
    #2 Rename the columns
        logger.info(f"perform_data_clean() is started for df_fact dataframe...")
        df_fact_sel = df_prescriber_fact.select(df_prescriber_fact.npi.alias("presc_id"), df_prescriber_fact.nppes_provider_last_org_name.alias("presc_lname"), \
                             df_prescriber_fact.nppes_provider_first_name.alias("presc_fname"), df_prescriber_fact.nppes_provider_city.alias("presc_city"), \
                             df_prescriber_fact.nppes_provider_state.alias("presc_state"), df_prescriber_fact.specialty_description.alias("presc_spclt"), df_prescriber_fact.years_of_exp, \
                             df_prescriber_fact.drug_name, df_prescriber_fact.total_claim_count.alias("trx_cnt"), df_prescriber_fact.total_day_supply, \
                             df_prescriber_fact.total_drug_cost)
